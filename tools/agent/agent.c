                                ...
                                const char *password_a)
{
    static const char script[] =
        ...
        " $mapped=$true\r\n"
        "}catch{\r\n"
        " Write-Output ('signed target=' + $remote + ' user=' + $env:ASB_SMB_USER)\r\n"
        " Write-Output ('signed error=' + $_.Exception.GetType().FullName + ': ' + $_.Exception.Message)\r\n"
        "}\r\n"
        "if(-not $mapped){\r\n"
        " try{\r\n"
        "  New-SmbGlobalMapping -LocalPath $local -RemotePath $remote -Credential $cred -Persistent $false -ErrorAction Stop | Out-Null\r\n"
        "  $mapped=$true\r\n"
        "  Write-Output 'plain mapping succeeded without RequireIntegrity'\r\n"
        " }catch{\r\n"
        "  $c=([int]$_.Exception.HResult -band 0xffff);if($c -eq 0){$c=1}\r\n"
        "  Write-Output ('plain error=' + $_.Exception.GetType().FullName + ': ' + $_.Exception.Message)\r\n"
        "  $h=($remote -split '\\\\')[2]\r\n"
