                                ...
                                const char *password_a)
{
    static const char script[] =
        ...
        "$mapped=$false\r\n"
        "$lastc=1\r\n"
        "foreach($n in @($env:ASB_SMB_USER,($h+'\\'+$env:ASB_SMB_USER))){\r\n"
        " foreach($sign in @($true,$false)){\r\n"
        "  if($mapped){continue}\r\n"
        "  try{\r\n"
        "   $c2=[System.Management.Automation.PSCredential]::new($n,$secure)\r\n"
        "   if($sign){New-SmbGlobalMapping -LocalPath $local -RemotePath $remote -Credential $c2 -Persistent $false -RequireIntegrity $true -ErrorAction Stop | Out-Null}\r\n"
        "   else{New-SmbGlobalMapping -LocalPath $local -RemotePath $remote -Credential $c2 -Persistent $false -ErrorAction Stop | Out-Null}\r\n"
        "   $mapped=$true\r\n"
        "   Write-Output ('mapped user=' + $n + ' signed=' + $sign)\r\n"
        "  }catch{\r\n"
        "   $lastc=([int]$_.Exception.HResult -band 0xffff);if($lastc -eq 0){$lastc=1}\r\n"
        "   Write-Output ('fail user=' + $n + ' signed=' + $sign + ' err=' + $_.Exception.Message)\r\n"
        "  }\r\n"
        " }\r\n"
        "}\r\n"
        "if(-not $mapped){\r\n"
        ...
        " try{\r\n"
        "  New-SmbMapping -LocalPath $local -RemotePath $remote -UserName $env:ASB_SMB_USER -Password $env:ASB_SMB_PASSWORD -ErrorAction Stop | Out-Null\r\n"
        "  Write-Output 'session-scoped New-SmbMapping SUCCEEDED (global mapping is the problem)'\r\n"
        "  Remove-SmbMapping -LocalPath $local -Force -ErrorAction SilentlyContinue\r\n"
        " }catch{\r\n"
        "  Write-Output ('session-scoped probe err=' + $_.Exception.Message)\r\n"
        " }\r\n"
        " exit $lastc\r\n"
        "}\r\n"
